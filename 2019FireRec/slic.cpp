...
void Slic::init_data(cv::Mat image) {
    ...
    for (int i = step; i < image.cols - step/2; i += step) {
        for (int j = step; j < image.rows - step/2; j += step) {
            ...
            cv::Vec3b intensity = image.at<cv::Vec3b>(nc.y, nc.x);
            cv::Scalar colour;
            colour.val[0] = intensity.val[0];
            colour.val[1] = intensity.val[1];
            colour.val[2] = intensity.val[2];
 ...
 */
double Slic::compute_dist(int ci, CvPoint pixel, cv::Scalar colour) {
 ...
 */
CvPoint Slic::find_local_minimum(cv::Mat image, CvPoint center) {
    ...
    for (int i = center.x-1; i < center.x+2; i++) {
        for (int j = center.y-1; j < center.y+2; j++) {
            ...
            cv::Vec3b intensity = image.at<cv::Vec3b>(j+1, i);
            cv::Scalar c1;
            c1.val[0] = intensity.val[0];
            c1.val[1] = intensity.val[1];
            c1.val[2] = intensity.val[2];

            /// CvScalar c2 = cvGet2D(image, j, i+1);
            intensity = image.at<cv::Vec3b>(j, i+1);
            cv::Scalar c2;
            c2.val[0] = intensity.val[0];
            c2.val[1] = intensity.val[1];
            c2.val[2] = intensity.val[2];

            /// CvScalar c3 = cvGet2D(image, j, i);
            intensity = image.at<cv::Vec3b>(j, i);
            cv::Scalar c3;
            c3.val[0] = intensity.val[0];
            c3.val[1] = intensity.val[1];
            c3.val[2] = intensity.val[2];
    ...
    for (int i = 0; i < NR_ITERATIONS; i++) {
        ...
        for (int j = 0; j < image.cols; j++) {
            for (int k = 0;k < image.rows; k++) {
                distances[j][k] = FLT_MAX; // maximum representable finite floating-point number
            }
        }
            ...
            for (int k = centers[j][3] - step; k < centers[j][3] + step; k++) {
                for (int l = centers[j][4] - step; l < centers[j][4] + step; l++) {
                    ...
                    if (k >= 0 && k < image.cols && l >= 0 && l < image.rows) {
                        ...
                        cv::Vec3b intensity = image.at<cv::Vec3b>(l, k);
                        cv::Scalar colour;
                        colour.val[0] = intensity.val[0];
                        colour.val[1] = intensity.val[1];
                        colour.val[2] = intensity.val[2];
        ...
        for (int j = 0; j < image.cols; j++) {
            for (int k = 0; k < image.rows; k++) {
                ...
                if (c_id != -1) {
                    ...
                    cv::Vec3b intensity = image.at<cv::Vec3b>(k, j);
                    cv::Scalar colour;
                    colour.val[0] = intensity.val[0];
                    colour.val[1] = intensity.val[1];
                    colour.val[2] = intensity.val[2];
...
void Slic::display_center_grid(cv::Mat image, cv::Scalar colour) {
    for (int i = 0; i < (int) centers.size(); i++) {
        cv::circle(image, cvPoint(centers[i][3], centers[i][4]), 2, colour, 2);
    }
}
